...
public class FriendlyAgitatorBehaviour : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!targetAcquired && readyToWork && References.targetElectors.Count > 0) //если нет цели
        {
            ChooseTarget(); //выбери
        }
        /*
        ...
        */
        if (targetAcquired) //если есть цель
        {
            ...
            if (!talking && Vector3.Distance(transform.position, myTarget.transform.position) < 1.3f) //если не говорим и мы достаточно близко
            {
                ...
                agent.enabled = false;              //вкапываемся
                myTarget.JoinTalk(gameObject);      //обращаемся к собеседнику
            }
            if (talking)
            {
                //Rotate
